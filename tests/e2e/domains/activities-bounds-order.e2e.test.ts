...
describe('Activities ordering & bounds E2E', () => {
  ...
  beforeAll(async () => {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      assert.equal((await agent.patch(`/chores/${c.json.uuid}/claim`).set({ Authorization: `Bearer ${tokens[alice.email]}`, Connection: 'close' })).status, 204);
      assert.equal((await agent.patch(`/chores/${c.json.uuid}/complete`).set({ Authorization: `Bearer ${tokens[alice.email]}`, Connection: 'close' }).attach('image', Buffer.from([1,2,3]), { filename: 'a.jpg', contentType: 'image/jpeg' })).status, 204);
    }
  }, 30000);
